...
void TestNumbers() {
    ...
    assert(Print(json::Node{-42}) == "-42"s);
    assert(Print(json::Node{-3.5}) == "-3.5"s);
...
void TestStrings() {
    ...
    std::string str = Print(str_node);

    assert( str == "\"Hello, \\\"everybody\\\"\""s);
...
void TestMap() {
    ...
    const json::Dict& dict = dict_node.AsMap();
    assert(dict.size() == 2);
...
void TestErrorHandling() {
    ...
    MustFailToLoad("\"hello"s);  // незакрытая кавычкаjson::
